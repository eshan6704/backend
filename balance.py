...
def fetch_balance(symbol):
    ...
    try:
        ...
        if df.empty:
            content_html = f"<h1>No balance sheet available for {symbol}</h1>"
        else:
            # Format numeric columns
            for col in df.columns:
                if ptypes.is_numeric_dtype(df[col]):
                    df[col] = df[col].apply(format_large_number)
            content_html = f"<div class='card'><h2>Balance Sheet</h2>{df.to_html(classes='styled-table', border=0)}</div>"
...
<html>
<head>
    <title>Balance Sheet for {symbol}</title>
    {STYLE_BLOCK}
</head>
<body>
    {content_html}
